#################################知识一（奖励数据的录制技巧）
有，而且你记得的方向基本对，不过官方更常用的说法不是“假成功/假失败数据”，而是：

false positives：明明没成功，分类器却判成成功
false negatives：明明成功了，分类器却没给成功

官方 HIL-SERL 网站先概括说，他们会先遥操作收集正样本和负样本，用来训练一个二分类 reward classifier。

更具体的技巧写在官方 franka_walkthrough.md 里，讲得很明确：

录 reward classifier 数据时，默认所有 transition 都记成负样本，只有你按键标记的那些才记成正样本。
官方明确建议：为了让分类器对 false positives 更稳，负样本最好比正样本多 2 到 3 倍。
而且负样本不是随便拍失败就行，应该专门覆盖容易误判的失败模式，例如：
插在错误位置
只插进去一半
拿着物体但还没真正插入
USB 任务里还特别提到：插进了错误的 USB 口 这种“很像成功但其实是失败”的情况。

这其实就是你说的“假成功/假失败”那个意思，只是官方没有用这个中文词。
更准确地说，官方是在强调：

要专门收集那些“看起来像成功、但实际上是失败”的负样本。

比如文档里就举例说：

如果分类器会把“RAM 条举在空中”误判成成功，那就要多收这种情况的负样本。

另外，官方还说在录 demo 时，如果你发现 reward classifier 出现 false positive 或 false negative，优先做法不是只调阈值，而是继续补针对性的 classifier 数据；必要时甚至增加 classifier 相机或图像。

所以一句话回答你：

有，官方确实强调了这种思路：reward classifier 要靠“成功样本 + 大量覆盖失败模式的负样本”来变稳，尤其要收那些“很像成功但其实失败”的样本，也就是你理解的那种“假成功/假失败”边界数据。


#########################################
知识一小结：成功状态要记录，失败状态要更多，而且要重点记录更多类似成功但其实失败的画面（如果已经训练出来的ckpt模型准确率因为这个原因不高，要重新记录更多这种数据）
#########################################



####
####
####


###########################################知识二（训练细节）
任务一旦判成功就结束并复位，那策略学到的到底是什么？部署时怎么连贯？

结论是：

官方这类 HIL-SERL 任务本来就是“按 episode 定义的单次任务”，不是一个无限连续流程。
也就是说，对 USB 插入这种任务，“插成功”本身就是这一回合的终点。demo 录制时，官方明确写的是：一旦 reward classifier 判成功，或者超时，机器人就 reset；录满指定数量的成功 demo 后脚本结束。RAM 插入和 USB pickup/insertion 两个 walkthrough 都是这么写的。

你自己的运行日志也符合这个逻辑：
actor 每个 episode 结束后会打印 [actor-episode-end]，然后立刻进入“正在准备复位 / 开始向底层发送复位坐标 / 单臂平滑复位”的流程。也就是说，你当前代码里，成功或 episode 结束后，后续动作不是“继续做下一个任务步骤”，而是 reset 到下一回合起点。

所以回答你最核心的问题：

1）成功后模型“怎么做”？

官方默认答案是：不继续做别的动作，而是把这一回合视为完成，然后 reset。

因为这类 task 的目标函数就是：

从初始状态出发
完成一次插入
成功即终止

在这种定义下，policy 学的不是“永远不停地工作”，而是：

给定一个新的初始状态，尽快把任务做成功。

这和工业里常见的“单工位循环任务”是一样的。不是策略自己负责决定“下一件事干嘛”，而是外部流程负责把环境恢复到下一个起点，再让策略重新执行一次。

####################################################
知识二小结：判断自己的模型和训练方式，是否成功后就复位，符合官网设计
######################################################


###
###
###

####################知识三（奖励和训练的相机不需要完全一致）
奖励的相机和训练的相机不需要一致（画面和个数）
（1）奖励的相机：只要可以准确识别成功画面即可
（2）覆盖整体状态（场景和夹爪）和局部状态（夹爪遮挡的部分）
##################################



###
###
###


########################知识四（动作缩放）
动作缩放
官方不是靠一个数学公式“算出来正确”，而是靠这类原则去定：

policy 输出的是小的 end-effector delta action
环境层把它缩放成真实机器人每控制周期的平移/旋转小增量
然后看它是否满足：
安全
稳定
能完成任务
不会抖动/冲过头/来不及到位

官方 repo 和说明里，动作都是这种 delta pose control 思路，不是直接输出绝对位姿。官方 issue 里也明确提到，SERL/HIL-SERL 的动作是小增量，因为每个控制周期都只走一点点。

所以“官网怎么知道是对的”，更准确地说是：

他们是通过真实机器人上的控制频率、动作平滑性、是否容易成功、是否会震荡或越界来调出来的。
不是随便填，也不是论文里给一个放之四海而皆准的固定值。


####


#####################知识五（扰动的设置）
增加reset的扰动
目前我增加了：
RANDOM_RESET = True
RANDOM_XY_RANGE = 0.01

而你的 GalaxeaUSBEnv._build_reset_target() 实现是：

先复制 RESET_POSE
如果 RANDOM_RESET=True
就只对 reset_pose[:2] 加一个 [-xy_range, xy_range] 的均匀随机扰动。

所以结论很明确：

你有没有加这种扰动？

有。

你加的是哪种扰动？

只加了 XY 平移扰动。

你有没有像官方一样加 RZ/朝向扰动？

目前没有。

因为我在你这份 GalaxeaUSBEnv._build_reset_target() 里只看到了：

reset_pose[:2] += ...

没有看到对旋转、尤其是 RZ 的随机化。


###
###
###


########################知识六（和官网的大致区别）
你当前 env 对训练效果最可能有影响的“大区别”

如果只挑最影响训练效果的，我会按这个顺序排：

1. reward 完全依赖 classifier，而不是 env 内部几何阈值

这是你和官网最大的训练机制差异之一。

2. state 比官网少很多

没有 vel / force / torque，这对插接任务可能很伤。



4. reset 少了官方的 recover / param switch / joint reset 周期

这会影响长时间稳定性和 reset 一致性。

5. reset 扰动只做了 XY，没有 RZ

鲁棒性分布比官网窄。

十五、给你一个非常直接的结论

你的 env 不是“和官网一样，只是把 Franka 换成了 Galaxea”。
它已经发生了这些级别的变化：

机器人通信后端变了
dual arm 架构变了
reward 放置位置变了
reward少了一种（我没有根据目标姿态来判定奖励，只靠视觉）
state 定义变了
gripper 控制语义变了
reset 机制变了
reset 扰动范围变了
camera pipeline 也变了

所以如果训练效果不好，不能只怪算法本身。
至少这一层 env，就已经和官网存在很多会影响学习行为的差异。


###
###
###
###################知识七（归一化理解）
模型输出一个归一化动作
例如单臂 7 维动作。
先 clip 到 [-1, 1]
防止输出过大。
把前 6 维解释成“这一步的末端位姿增量”
平移增量：action[:3] * POS_SCALE
旋转增量：action[3:6] * ROT_SCALE
把这个增量加到“当前末端位姿”上
也就是：
next_xyz = current_xyz + delta_xyz
next_quat = 当前姿态 + 旋转增量（通过 apply_delta_rotation）
再做安全裁剪
新目标位姿还会过工作空间限制，避免超边界。
最后发送给 ROS / 机械臂
这个“裁剪后的目标末端位姿”才是最终发下去的控制目标。

所以你这句话：

clip再动作缩放后是一次输出的位置增量，再转换成最终的末端位姿吗？

更准确地说是：

先 clip，再缩放得到“本步末端位姿增量”，再加到当前末端位姿上，得到最终目标末端位姿。

再补一个关键点：

平移/旋转 是这样走“增量 -> 目标位姿”的
夹爪 不是这样，它不是连续位姿增量，而是：
-1 -> close
0 -> hold
+1 -> open
然后再映射成硬件量程。

还有一个例外你要记住：

reset 不走这条增量链路。
reset 是直接给绝对目标位姿，不是模型输出动作再 clip/scale。




###
###
###


########################知识八（bug）

#####
      （bug1）
如果Mode 2 / script_control_enabled=True 时继续 env.step()，同时等待切回 Mode 0，会造成等待mode0前末端自己运动。
如果Mode 2 / script_control_enabled=True 时不再继续 env.step()，只等待切回 Mode 0。末端不会自己动。
目前demos脚本和奖励数据脚本都这么修改从而不让末端自己移动了


bug来源可能：
reset后，等待mode0之前，会发布：
actions = np.zeros(...)
env.step(actions)（零增量的意思）
发布令零增量从而保持末端不动，但是我怀疑，零增量仍然会自己动，从而造成末端自己慢慢微小运动。


解决了：末端在reset后，等待mode0之前，末端自己慢慢移动的bug就是所谓的reset后发布0增量所谓维持原位置是错误的，

验证办法：reset后，等待五秒去处发布零增量，五秒后开始发布零增量，且不打开mode0,发现确实一发布零增量就开始移动。

所以不影响actor，因为actor不发布零增量，发布的是网络认为应该输出的增量，所以不会发生这个问题。
所以只在demos脚本和奖励数据脚本去除发布零增量等待mode0,改成完全静默无动作，等待mode0来临即可。


#####
      （bug2）

demos数据可视化后可以发现，把reset过程中的一帧进行录制了，不知道影响大不大？


#####
#####
#####

########################知识九（归一化后的动作缩放）

调试归一化思路：
    网络/脚本输出 1、0.5、-1 这些归一化动作后，底层目标位姿是否真按 0.01 m / 0.05 rad 这个比例变化。”
    不必纠结是不是把末端位姿归一化【-1,+1】，是输出增量，输出增量+1,对应（+1）*action_scale
    末端位姿起基准作用，在反馈的末端位姿下进行增量控制
      更准确地说：

       不是把“末端位姿本身”归一化到 [-1,+1]
       而是把动作增量定义在 [-1,+1]
       然后进入 env 后：
       平移增量 = action[:3] * action_scale
       旋转增量 = action[3:6] * action_scale
       再把这个增量加到当前反馈/当前记录的末端位姿上
       得到下一步目标末端位姿

所以：

当前末端位姿是基准，归一化的是“本步增量动作”，不是“绝对位姿”。

调试步骤：
  （1）确认确实归一化+真的应用了动作缩放
  （2）如果 POS_SCALE = 0.01   +ROT_SCALE = 0.05   # 0.05 rad，那就是归一化后输出1就是移动0.01m/转动0.05rad。


测试结果（+-1归一化+位置0.01+旋转0.05的缩放）：最终判断是：
  归一化到缩放这条链没大问题，特别是 x 和 rx/ry/rz 已经验证得很漂亮；但在你这个 reset 姿态附近，y、z 方向的真实执行有明显耦合和方向不对称，尤其正方向更明显。


  ###
  ###
  ###

  ###############################知识十（存入的buffer和demo_buffer数据分析）

  buffer 输出说明三件事：

reward / done / mask 存储是对的
所以 return=1 的成功 transition 没丢。
success=0 是日志/info 字段没统一，不是 reward 没触发。
夹爪三值存储是对的，但前 6 维动作存在超过 [-1,1] 的问题，需要在 actor 存 buffer 前 clip。

你现在最应该立刻修的是：

success 字段统一 + stored_actions[:6] clip 到 [-1,1]


####
####
####


###############################知识十一（相机输入）
  总结：按照奖励数据，demos，rlpd需要最大的相机打开ENV_IMAGE_KEYS
           即：
              （1）都使用最大个数相机录制数据，
              （2）rlpd和classifer时选择合适的相机作为输入。

具体任务的config：
###表示本任务环境实际打开/采集哪些相机。后面的 demo、reward classifier、RLPD policy 都只能从这里面选。所以它应该覆盖本任务所有脚本里需要用到的最大相机集合。
1,ENV_IMAGE_KEYS = ["head_rgb", "left_wrist_rgb", "right_wrist_rgb"]

###表示 RLPD actor/critic 策略网络实际吃哪几路图像。demo 里可以有三路，但只要至少包含这里需要的两路，RL 训练就可以只用两路。
2,image_keys = ["head_rgb", "right_wrist_rgb"]

###表示奖励分类器实际吃哪几路图像。你 classifier 是三路训练出来的，所以 RLPD 在线训练时也要给它三路 obs。
3,classifier_keys = ["head_rgb", "left_wrist_rgb", "right_wrist_rgb"]


###
###
###


#######################知识十二（训练速度提升）

（1）ENV_IMAGE_KEYS多+image_keys 少，可以提升learner+actor侧训练速度。
（2）ENV_IMAGE_KEYS少+image_keys 少，可以进一步提升actor训练速度（减轻了图像处理压力），learner侧几乎不影响。
   但是classifer必须重新训练成ENV_IMAGE_KEYS对应的相机个数
  
（3）有时候actor输出动作特别慢：考虑ssh连接问题  +  服务器显卡和别人公用，功率极大下降


###
###
###

#####################知识十三（检查数据）


（1）检查demos图像和数据
（2）检查buffer的图像和数据，归一化，夹爪，奖励
（3）图像不要公用，各取所需，采集数据的时候也是


细致检查分析：
 （1）检查demos数据时，发现动作归一化、夹爪三值、图像裁剪、成功终止标记都正常。唯一建议是留意 dy 全部等于 1.0 的动作饱和现象，但这不是数据格式 bug。是因为我录制时，+y的方向进行了快速移动，是否我录制demos时应该，全程匀速运动呢？不应该快速移动+y？
   后续如果训练中出现动作过猛、接触不稳，可以考虑重录更慢、更平滑的 demos，或者降低 POS_SCALE / 调整 VR 映射。





####
####
####



###########################知识十四（夹爪详细分析）


（1）三值标签：+-1,0分别表示张开，闭合，保持上一状态不动
（2）夹爪惩罚：单独对夹爪的离散dqn奖励计算起作用，不要影响末端的reward
（3）夹爪惩罚处罚条件：夹爪状态反转则处罚，抓物体夹爪状态反转不是很正常，确实正常的，一个任务就几次是合适的，其他时间都是错误的，所以即使正确抓物体也要给惩罚，其他时候惩罚更多，表示不要轻易反转状态。


（4）夹爪三值定义还需要进一步确认准确度和定义的合理性，
demos 和 RLPD 的夹爪保存定义是统一的。
量程反馈用于判断真实夹爪状态；前后状态变化再生成 -1/0/+1 事件标签。
这种方法准确率通常比直接用动作命令高，但要确认 close_max=30 能覆盖“夹住 USB 后”的实际反馈值。
   由于标签准确率高，所以wrapper基于标签给夹爪惩罚的定义也准
    真实夹爪反馈量程
        ↓
    <=30 判定为 closed
    >=70 判定为 open
    30~70 保持上一稳定状态（为了覆盖量程，这个范围几乎不会存在，迅速掠过）
        ↓
    比较 obs -> next_obs 的状态变化
        ↓
    生成 action[6] 事件标签：
     -1 = close event
      0 = hold/no-op
     +1 = open event

这个比直接相信 policy_raw 或 intervene_action 更可靠，因为它记录的是夹爪实际发生了什么，不是“命令想让夹爪做什么”。
（但要加一个前提：夹爪反馈本身可靠，且 CLOSE_MAX=30、OPEN_MIN=70 阈值适合你的真实硬件。(可视化观察闭合后夹爪量程是否仍小于30)）



###
###
###



###########################知识十五（wandb）

learner端支持wandb，先登陆：
（1）wandb ：
wandb status
wandb login --verify
wandb online
（2）输入key：loginwandb_v1_8Uf8krEtedBNwL62O6QBNeaBoRK_miAJZd1wqF00VrFsvbfxKCcOXUITWtOVeOUvkFIPAR40sWZnT
（3）输入指令：
python train_rlpd.py   --exp_name=galaxea_usb_insertion_single   --learner=True   --ip=localhost   --demo_path=./demo_data_single   --checkpoint_path=./rlpd_checkpoints_single   --debug=False

###
###
###


##########################  与官网区别（2026.4.25）

第二：process_demos() 过滤零动作。
这个可能误删 hold/成功附近静止帧。建议改成 return list(transitions)。

第三：动作缩放和限位。
POS_SCALE/ROT_SCALE 是环境语义，必须固定；它和官网不同没问题，但改了就要重录/清 buffer。

第四：reset 起点分布。
你的 VR Mode 2 + script reset 和官网 Franka reset 差异大。只要 reset 稳定且不污染数据，就可以；否则训练会很受影响。

第五：proprio 更少。
你少了速度/力/力矩，可能影响接触插入阶段学习。不是必须改，但如果接触阶段学不好，这是候选原因。

第六：夹爪 penalty。
现在 wrapper 思路已经和官网一致，主要剩下 penalty 大小和阈值适配。这个可以后看，不是当前最大差异。


你的最新 train_rlpd.py 已经包含：
✅ actor_to_learner 图像裁剪
✅ 在线 action 归一化
✅ 在线 gripper 事件标签重写
✅ demos 加载清洗
✅ actor/learner 网络同步稳定逻辑
✅ grasp_penalty 单独存入 transition

目前这份 RLPD 从结构上看，和我们前面讨论的最终设计是一致的。真正需要继续重点检查的就不是“RLPD 是否缺关键逻辑”，而是：

1. ✅config 里的 process_demos 是否还过滤零动作，
   #########应该过滤，
   #########目前我的demos完整保留操作轨迹，不过滤静止帧，
   #####现在的录制脚本已经解决了 reset/等待帧问题：它明确要求“第一次 VR 接管后才开始记录”，而且说明“全程轨迹完整保留，不删除演示开始后的静止帧”。开始记录后，如果某帧没有接管，它会保存零动作 zero_after_recording_started，再写入 trajectory。
   ####所以录制demos时，vr暂停，会把无vr接管时的动作级成零增量的静止帧，后续learner应该去掉这个静止帧。
   #########只要保证demos时，不是帧的需要静止帧，即可以删去。
   ###########1e-4 是几乎全静止阈值。
       过滤它可以减少无意义零动作帧，
          但不能无脑过滤全部静止帧。
           你应该至少保留 reward=1 和 done=True 的静止帧，
                否则可能删掉最关键的成功终止数据。

2. ✅ wrapper 里的 grasp_penalty 是否只写 info、不改 reward  
3. ✅ env.step 内部是否正确把 -1/0/+1 夹爪标签映射到硬件 10/保持/80
4. ✅ （抓镊子后保持量程20,算闭合）CLOSE_MAX=30 / OPEN_MIN=70 是否适合真实夹持反馈



####
####
####


###########################目前我认为的进度

是的，我现在的判断是：你前面列出的那些“框架级问题”，基本都已经符合要求了。

具体说：

RLPD 数据链路是对的：你的最新 train_rlpd.py 已经有 actor→learner 图像裁剪、动作归一化、在线夹爪三值事件重写、demo/buffer 加载前 action sanitize，这些都符合我们前面定下的设计。
demos 录制脚本不再过滤静止帧：它是“第一次 VR 接管后才开始记录”，开始记录后即使没有接管，也会保留零动作/hold 帧，这符合你要的完整演示轨迹逻辑。
RLPD 没有调用 config 里的 process_demos() 静止帧过滤：也就是说 config 里那段过滤逻辑目前不会影响 learner 加载 demo。你的 RLPD 是自己读取 pkl、sanitize、prune、再 insert 到 demo_buffer。
grasp_penalty 已经改成只写 info，不改主 reward：你的 SingleGripperPenaltyWrapper 里现在是 info["grasp_penalty"] = float(penalty_val)，最后返回原始 reward，没有再 reward += penalty_val。这符合官网 learned-gripper 的分支惩罚思想。
config 的相机分工也合理：ENV_IMAGE_KEYS 打开三路，image_keys=["head_rgb", "right_wrist_rgb"] 给 RLPD policy，classifier_keys=["left_wrist_rgb"] 给奖励分类器，环境组装时也是先加载 classifier wrapper，再加 gripper penalty wrapper。

所以现在不是“框架还有明显错误”的阶段了，而是进入了训练效果调参阶段。

后面主要看这些细节：

1. reward classifier 是否稳定、阈值 0.7 是否合适
2. demos 质量是否足够，动作是否太饱和、太快或成功样本太少（不要让归一化动作一直输出1这种，）
3. CLOSE_MAX=30 / OPEN_MIN=70 是否覆盖真实夹持反馈（该次任务符合）
4. POS_SCALE / ROT_SCALE 是否让策略动作既够细又不太慢（调试脚本是符合，训练还需感受）
5. 左腕单相机作为 reward 判断是否有误判角度  （目前没有，如果多次出现，可以增加成功附近的状态重新训练）
6. RLPD 中 online 数据和 demo 数据比例、训练 loss、grasp_critic loss 是否正常

一句话：现在整体框架我认为已经没有明显结构性问题了，剩下主要就是训练效果、数据质量、阈值和超参数的细节优化。



###
###
###


#######################知识十六（vr的归一化）

之前的归一化测试脚本，主要测试的是：

policy/VR action[:6] ∈ [-1, 1]
经过 env 内部 POS_SCALE / ROT_SCALE
最终机器人末端实际走多少

现在这个问题是更上游的：
你的 VRInterventionWrapper 当前把 VR 手柄的绝对位置/角度直接拼成 action，所以会出现 1.58 这种超过 [-1,1] 的值。你需要在 VR wrapper 源头加一层：

VR 当前手柄位姿 - 进入 VR 接管瞬间的手柄位姿
再除以 vr_pos_range / vr_rot_range
最后 clip 到 [-1,1]

也就是：先把 VR 输入归一化成 policy 动作空间，再交给 env.step。


所以我需要调试任务适配的vr动作缩放，当demos脚本不报错超限，则动作缩放适配当前任务



###
###
###

##########################最新输出(2026.4.26)


（1）rlpd的actor的buffer和demobuffer：
唯一建议：注意早期几条接近饱和的动作

虽然没有越界，但普通 buffer 里有几条动作很接近边界，例如 idx=121、idx=134、idx=711 这几条。它们的前 6 维有接近 ±1.0 的值。

这不影响数据合法性，但后续训练时你可以留意两点：

1. 如果只是少量探索/人工接管动作接近 1，可以接受。
2. 如果 wandb 或后续 inspect 发现大量动作长期接近 ±1，说明策略或 VR 接管动作可能过猛，需要调小动作尺度或改善策略稳定性。

目前从总体统计看，它不是格式 bug，也不是必须立刻改代码的问题。

最终结论

这份最新 RLPD buffer 检查结果可以通过。

普通 buffer 和 demo_buffer 都已经满足正式训练数据要求：

✅ reward / done / mask 正确
✅ 成功 transition 保存正确
✅ action shape 全部为 7
✅ 前 6 维动作没有越界
✅ gripper 是 -1/0/+1 事件标签
✅ grasp_penalty 与最终 action[6] 完全对齐
✅ close/open 才有 penalty
✅ hold 没有 penalty
✅ 图像选择符合 head_rgb + right_wrist_rgb + state

一句话：RLPD actor 在线保存 buffer 的核心数据链路已经正确，剩下主要关注训练效果和动作是否过于激进。

（2）demos全部正常

（3）wandb的训练日志（运行9min的）：


###
###
###


#########################知识十七（actor的x/1000000的x卡顿时，不要vr遥操）
如果卡住几秒期间你完成了一组成功 VR 遥操，中间关键动作大概率不会完整进入 buffer/demo_buffer。这会让成功数据变断层，尤其会影响模型学习“如何夹住物体”。

所以你现在应该优先保证 VR 采样连续性：
（办法1）把网络更新频率降低，比如 client_update_period=50 或 100，不要为了频繁拿新网络牺牲人类示范数据质量。

（办法2）vr遥操时，不接受新的learner发出的网络，但是比较复杂，因为learner固定发布网络，要是actor不接受，可能出现新的问题,




###
###
###

##########################知识十七（增量控制）

如何记录的增量控制

是否可以把记录的增量控制输出出去，让我看记录的增量+归一化+动作缩放是否合理

不知道的东西：怎么收到我的手柄位姿？ee-pose可以，vrpose里的位姿不对不可用
修改了wrappers不读取vrpose，用末端反馈



###
###
###


########################知识十八（改进）

（1）取消记录vr手柄位姿（手柄位姿发漂）
后续 inspect 要看有没有大量 ±1。少量饱和可以接受，大量饱和说明 actor step 太慢、VR 速度太快，或者 scale 太小。

（2）解决vr介入时actor记录不全动作（不要卡顿时候有vr数据）
分析：我的vr优先级很高，因为只要切换mode0立马vr接管，而官网的是正常actor输出，需要接管立马移动摇杆，而我的必须先切换到mode0.
（3）打印critic评分，一直是0之类的肯定不对
（4）播放轨迹动作，看看记录的数据是不是对的
（5）增加训练时间
（6）增大buffer容量
（7）增加demos条数





####
####
####


###################知识十九


最新改动：改动了不用vr手柄计算增量，使用末端反馈反推增量。
   执行replay_demo_action脚本，回放动作，计算出合适的动作缩放
   action[:6] normalized
= demo 里保存的归一化动作
≈ actor 网络未来会输出的动作格式

expected pos/rot delta
= action[:6] * POS_SCALE / ROT_SCALE
= 理论上 env 想让机器人执行的真实增量

live obs-delta action[:6]
= 真机 replay 后，实际末端运动 / POS_SCALE / ROT_SCALE
= 真实执行结果反推出来的动作

所以，action[:6] normalized 相当于 actor 网络输出的动作格式；expected pos/rot delta 才是缩放后的理论真实动作；live obs-delta action[:6] 才是真机最终实际走出来的动作结果。
demo max_abs_error 基本接近 0
live max_abs_error mean/max = 0.176673 / 0.809571
live_error_warn_count = 31

这说明 demo 内部记录是自洽的，也就是 demo action 和 demo 记录的末端前后位姿能互相反推上；CSV 里也能看到 action、expected delta、demo_delta_action 这些字段正是这个对应关系。
但是 真机 replay 时，env.step(action) 后实际末端没有完全走到 expected delta。这就是你说的“动作移动距离变小，没有到达真实位置”。

1. 可以根据 replay 结果计算正确缩放吗？

可以，但只能在“误差主要是稳定比例误差”的情况下可靠。



#####
####
#####


###############知识二十

(1)执行replay_demo_action时，夹爪好像不能完全闭合呢？不是执行量程10的指令吗？
(2)微小动作范围输出实际动作抖动更大


###
###
###


#####################最新修改

（1）vr手柄位姿完全取消
（2）reset/episode结束时才开始接受learner的网络
（3）增加demos共40条（但是回放demos时动作被缩水不能呢改完全复原，我不知道是好是坏）


接下来的问题：
（1）动作缩放让demos复原
（2）动作频率
（3）退出vr后，夹爪会稍微张开，不知道为什么
（4）actor输出奇怪动作，多次干预到正确路径后，会纠正过来。
（5）虽然增量控制，但是没有绝对位姿概念吗？


目前最好的效果： 向下，向前，抓取，向上。
基本学会： 选择抓取后一般不会在打开
趋势：输出的动作大多数都是demos里有的路径



目前实现：

（1）learner和actor的互相通讯百分百实现
（



改进：
（1）求增量：
     方法一：用反馈信息求增量（和实际效果一致，如果忽略延时误差等实际执行损失，可近似人类需求）
     方法二：用实际指令求增量（和人需求一致，实际效果不记录）




目前回放和actor的动作输出频率3.7hz左右，所以动作执行比较慢，但是actor实际输出动作较大，而回放的动作较慢，动作无幅度较小：
  原因：
之前 actor 看起来快，不是因为 step 频率高。
step 频率大约 3.7Hz，其实不高。

真正原因是：
actor 输出的部分 action 维度比较大，
有些旋转维度甚至接近 -1 / +1，
每步物理增量较大，
连续几步叠加后机器人看起来动作快。

所以实际actor输出符合频率和延迟（验证确实是频率较低，动作幅度大纯粹是因为增量输出大，因为action快饱满了，接近归一化的+-1，可以保证每步尽量走满，只是action接近饱和+-1，动作幅度大。


###
###
###


###################新思路

（1）虽然reset后不会env（zero）让末端慢慢移动，但是进入mode0再切换会mode2后，仍会慢慢移动，且出现夹爪微微张开又闭合，不知道影不影响标签，从幅度来看应该不影响，但是会造成训练过程中微微张开导致物体掉落

（2）回放buffer的数据，

（3）录制阶段，无速度控制，但是输出指令后，末端有一个速度控制，看看有没有这个参数，是否可以调小是actor输出的动作增量没那么大。

（4）是否可以录制demos一个动作缩放，actor又一个动作缩放，让actor输出的动作增量没那么大？

（5）虽然录制demos的增量是先记录绝对位姿再计算增量的，但是rlpd的buffer不是！！！！
所以vr介入的必须读反馈末端（录制demos脚本验证了读取反馈可以输出更稳定）然后episode结束后瞬间转换成需要的增量信息。
policy输出的归一化暂时不要用反馈再转换，因为sac要评价输出的action，


（6）写一个脚本，把本次episode的保存的数据保存到rlpd_test_buffer文件夹下，然后再播放，看看增量，夹爪，是不是可以完整复现


下一步：
解决夹爪微微张开（先打印切换这几帧的完整夹爪量程，标签，如果不影响存入的buffer，则可以忍受，只是实际训练的时候微微张开会造成物体掉落）（确认是17.73 -> 20.81 -> 19.87 -> 18.60 -> 17.95，微小bug，不影响训练的夹爪标签，不优先处理）

录制训练中的episode，可视化+回放，看看逻辑是不是对的上





###
###
###



#################
一会：
（1）分析输出
（2）由于actor频率变快，更容易到达最大步数从而reward=0,是否要出现reward=0的情况呢。
（3）频率加快（15hz，延迟=0,频率可达到10-15hz，然后对actor输出的action进行比例缩小，从而动作增量变小）
