...
class GalaxeaArmEnv(gym.Env):
    ...
    def __init__(self, config, cfg, save_video=False):
        ...
        # 10.1 夹爪执行诊断日志
        # ----------------------------------------------------------
        # 只打印/记录，不改变任何动作执行、buffer 标签或训练逻辑。
        # 目的：确认 Mode0 -> Mode2 后到底有没有发布 open_cmd=80，
        # 以及 actor 准备写入 buffer 的 action[6] 标签是否保持正确。
        # ==========================================================
        self.gripper_diag_enable = bool(
            getattr(self.config, "GRIPPER_DIAG_ENABLE", False)
        )
        self.gripper_diag_print_every_step = bool(
            getattr(self.config, "GRIPPER_DIAG_PRINT_EVERY_STEP", False)
        )
        self.gripper_diag_print_only_on_change = bool(
            getattr(self.config, "GRIPPER_DIAG_PRINT_ONLY_ON_CHANGE", True)
        )
        self.gripper_diag_print_prefix = str(
            getattr(self.config, "GRIPPER_DIAG_PRINT_PREFIX", "[gripper-publish]")
        )
        self._gripper_publish_step = 0
        self._last_gripper_map_debug = {}
        self._last_gripper_publish_debug = {}
        self._last_gripper_diag_signature = None
    ...
    def _describe_gripper_cmd(self, value):
        if value is None:
            return "None"
        try:
            x = float(value)
            ...
            return repr(value)
        if 0.0 <= x <= 100.0 and abs(x) > 5.0:
            if x <= self.gripper_feedback_close_max:
                return f"hw_close({x:.2f})"
            if x >= self.gripper_feedback_open_min:
                return f"hw_open({x:.2f})"
            return f"hw_middle({x:.2f})"
        if x <= -0.5:
            return f"close_event({x:+.3f})"
        if x >= 0.5:
            return f"open_event({x:+.3f})"
        return f"hold({x:+.3f})"

    def _snapshot_gripper_exec_state(self, arm_side: str):
        return {
            "last_hw_cmd": float(self._last_hw_gripper_cmd.get(arm_side, self.gripper_open_cmd)),
            "has_explicit": bool(self._gripper_has_explicit_command.get(arm_side, False)),
            "latch_kind": self._gripper_latch_kind.get(arm_side, None),
            "latch_remaining": int(self._gripper_latch_remaining.get(arm_side, 0)),
            "latch_cmd": float(self._gripper_latch_cmd.get(arm_side, self.gripper_open_cmd)),
        }

    def _get_current_gripper_feedback_from_last_state(self, arm_side: str):
        try:
            return self._extract_gripper_feedback_from_state(self._last_valid_state, arm_side)
        except Exception:
            return None

    def _should_print_gripper_diag(self, record):
        if not self.gripper_diag_enable:
            return False
        if self.gripper_diag_print_every_step:
            if not self.gripper_diag_print_only_on_change:
                return True
            sig = (
                record.get("arm_side"),
                round(float(record.get("raw_cmd", 0.0)), 4),
                round(float(record.get("mapped_hw", 0.0)), 4),
                record.get("feedback_before"),
                record.get("map_reason"),
            )
            changed = sig != self._last_gripper_diag_signature
            self._last_gripper_diag_signature = sig
            return changed
        return False

    def _print_gripper_publish_diag(self, record):
        if not self._should_print_gripper_diag(record):
            return
        before = record.get("memory_before", {}) or {}
        after = record.get("memory_after", {}) or {}
        print(
            f"{self.gripper_diag_print_prefix} "
            f"step={record.get('publish_step')} arm={record.get('arm_side')} "
            f"raw_action6={record.get('raw_cmd'):+.4f} "
            f"raw_label={record.get('raw_label')} "
            f"map_reason={record.get('map_reason')} "
            f"mapped_hw={record.get('mapped_hw'):.2f} "
            f"feedback_before={record.get('feedback_before')} "
            f"mem_before={before.get('last_hw_cmd')} "
            f"mem_after={after.get('last_hw_cmd')} "
            f"explicit_before={before.get('has_explicit')} "
            f"explicit_after={after.get('has_explicit')} "
            f"latch_before=({before.get('latch_kind')},{before.get('latch_remaining')},{before.get('latch_cmd')}) "
            f"latch_after=({after.get('latch_kind')},{after.get('latch_remaining')},{after.get('latch_cmd')})"
        )
    ...
    def _map_gripper_cmd_to_hardware(self, cmd: float, arm_side: str) -> float:
        ...
        额外记录 self._last_gripper_map_debug[arm_side]，只用于诊断。
        """
        cmd = float(cmd)
        memory_before = self._snapshot_gripper_exec_state(arm_side)
        feedback_before = self._get_current_gripper_feedback_from_last_state(arm_side)
        raw_label = self._describe_gripper_cmd(cmd)

        if 0.0 <= cmd <= 100.0 and abs(cmd) > 5.0:
            hw = float(cmd)
            self._remember_explicit_gripper_cmd(arm_side, hw)
            map_reason = "direct_hardware_range"
        elif cmd >= 0.5:
            ...
            map_reason = "open_event_to_open_cmd"
        elif cmd <= -0.5:
            ...
            map_reason = "close_event_to_close_cmd"
        else:
            hw = self._resolve_hold_gripper_cmd(arm_side)
            self._last_hw_gripper_cmd[arm_side] = hw
            map_reason = "hold_to_memory_or_latch"

        memory_after = self._snapshot_gripper_exec_state(arm_side)
        rec = {
            "arm_side": arm_side,
            "raw_cmd": float(cmd),
            "raw_label": raw_label,
            "mapped_hw": float(hw),
            "mapped_hw_label": self._describe_gripper_cmd(hw),
            "map_reason": map_reason,
            "feedback_before": None if feedback_before is None else float(feedback_before),
            "memory_before": memory_before,
            "memory_after": memory_after,
        }
        self._last_gripper_map_debug[arm_side] = rec
        return float(hw)
    ...
    def step(self, action: np.ndarray):
        ...
        info = {
            "gripper_debug": self._get_gripper_debug_info(),
            "gripper_publish_debug": copy.deepcopy(self._last_gripper_publish_debug),
            "step_timing": {
                "hz": float(self.hz),
    ...
    def _publish_action(self, action: np.ndarray):
        ...
        if self.arm_mode == "dual":
            ...
            self._gripper_publish_step += 1
            left_rec = dict(self._last_gripper_map_debug.get("left", {}))
            right_rec = dict(self._last_gripper_map_debug.get("right", {}))
            left_rec.update({"publish_step": self._gripper_publish_step, "published_hw": float(hw_left)})
            right_rec.update({"publish_step": self._gripper_publish_step, "published_hw": float(hw_right)})
            self._last_gripper_publish_debug = {"left": left_rec, "right": right_rec}
            self._print_gripper_publish_diag(left_rec)
            self._print_gripper_publish_diag(right_rec)
            ...
            self._gripper_publish_step += 1
            rec = dict(self._last_gripper_map_debug.get(self.arm_side, {}))
            rec.update({"publish_step": self._gripper_publish_step, "published_hw": float(hw_gripper)})
            self._last_gripper_publish_debug = {self.arm_side: rec}
            self._print_gripper_publish_diag(rec)
