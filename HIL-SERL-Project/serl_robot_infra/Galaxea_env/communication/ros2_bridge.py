...
# from communication.robot_topics import RobotTopicsConfig
# from communication.message_queue import MessageQueue
# from utils.message_convert import (
#     header_stamp_to_timestamp,
#     pose_to_7d_array,
#     compressed_image_to_rgb_array,
#     array_to_joint_state
# )
# from utils.datatype import RobotAction

# 将这几行全部改为绝对路径
from serl_robot_infra.Galaxea_env.communication.robot_topics import RobotTopicsConfig
from serl_robot_infra.Galaxea_env.communication.message_queue import MessageQueue
from serl_robot_infra.Galaxea_env.utils.message_convert import (
    header_stamp_to_timestamp,
    pose_to_7d_array,
    compressed_image_to_rgb_array,
    array_to_joint_state
)
from serl_robot_infra.Galaxea_env.utils.datatype import RobotAction
...
class Ros2Bridge:
    ...
    def get_latest_obs_forcely(self, device: torch.device = torch.device("cpu")):
        """异步模式核心：不进行时间对齐，强制抓取每个话题的最新数据"""
        obs = {"images": {}, "state": {}}
        
        # 必须确保所有关键传感器至少有一帧数据
        for name, buffer in self.obs_buffer.items():
            if len(buffer) == 0:
                # 只要有一个 buffer 为空（比如相机还没出图），就返回 None 让上层继续等待
                return None
            
            latest_msg = buffer[-1]
            data = latest_msg["data"]
            
            # 统一转化为 Torch Tensor 格式
            if not isinstance(data, torch.Tensor):
                data = torch.from_numpy(data).unsqueeze(0) if hasattr(data, '__array__') else torch.tensor(data)
            
            if name in self.topics_config.images:
                obs["images"][name] = data.to(device)
            else:
                obs["state"][name] = data.float()
