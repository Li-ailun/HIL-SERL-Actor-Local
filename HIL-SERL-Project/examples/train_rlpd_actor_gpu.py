...
def actor(agent, data_store, intvn_data_store, env, sampling_rng, config, trainer_cfg):
    ...
    if FLAGS.eval_checkpoint_step:
        ckpt = checkpoints.restore_checkpoint(os.path.abspath(FLAGS.checkpoint_path), agent.state, step=FLAGS.eval_checkpoint_step)
        ...
        for episode in range(FLAGS.eval_n_trajs):
            ...
            while not done:
                ...
                policy_obs = prune_observation_for_actor_to_learner(
                    obs, actor_to_learner_image_keys, config, strict=FLAGS.actor_to_learner_strict_keys
                )
                actions = agent.sample_actions(observations=jax.device_put(policy_obs), argmax=False, seed=key)
    ...
    if FLAGS.ip == "localhost" and getattr(trainer_cfg, "broadcast_port", None):
        ...
        _log_info("actor_warning", f"[actor-broadcast-hint] {_suggest_ssh_forward_command(trainer_cfg.port_number, trainer_cfg.broadcast_port)}", "yellow")
    ...
    def _wait_for_network(reason, *, require_new=NETWORK_WAIT_REQUIRE_NEW, timeout_sec=NETWORK_WAIT_TIMEOUT_SEC):
        ...
        _log_info(
            "actor_network",
            f"[actor-network-wait] reason={reason}, before_recv={before}, require_new_broadcast={require_new}, timeout_sec={timeout_sec}",
            "blue",
        )
        while True:
    ...
    try:
        for step in pbar:
            ...
            # 重要：episode 内不 client.update、不 apply pending、不后台 update。
            with timer.context("sample_actions"):
                if step < config.random_steps:
                    ...
                    policy_actions = current_agent.sample_actions(
                        observations=jax.device_put(policy_obs), seed=key, argmax=False
                    )
            ...
            with timer.context("step_env"):
                ...
                if had_intervene_action:
                    ...
                    _, prev_exec_gripper_cmd = map_single_arm_exec_action_to_hardware(stored_actions, prev_exec_gripper_cmd)
                ...
                actions, stable_gripper_state = rewrite_single_arm_gripper_action_to_three_value(
                    stored_actions, obs, next_obs, stable_gripper_state
                )
                ...
                next_obs_to_store = prune_observation_for_actor_to_learner(
                    next_obs, actor_to_learner_image_keys, config, strict=FLAGS.actor_to_learner_strict_keys
                )
                transition = dict(
                ...
                if (not FLAGS.minimal_logs) and step % FLAGS.print_period == 0:
                    dbg_exec_actions, _ = map_single_arm_exec_action_to_hardware(policy_actions, prev_exec_gripper_cmd)
                    since_last_recv = None if network_debug["last_recv_time"] is None else round(time.time() - network_debug["last_recv_time"], 3)
                    print_blue(
                        ...
                        f"grasp_penalty={transition.get('grasp_penalty', 'N/A')}, had_intervene_action={had_intervene_action}"
                    )
                ...
                if done or truncated:
                    ...
                    try:
                        ...
                        _log_info("actor_warning", f"[actor-send-stats-warning] {e!r}", "yellow")
        ...
        # 旧版 buffer 逻辑：只保存周期整点文件 transitions_1000.pkl / 2000.pkl / ...
        # 退出时不再保存 transitions_XXX_final.pkl，避免下次 start_step 解析和 learner 恢复读入混乱。
...
#         _log_info(
#             "actor_network",
#             f"[actor-network-wait] reason={reason}, before_recv={before}, require_new={require_new}, timeout_sec={timeout_sec}",
#             "blue",
#         )
#         while True:
#             _client_update(reason, force_print=False)
...
#                 _log_info(
#                     "actor_network",
#                     f"[actor-network-wait-done] reason={reason}, recv_count={after}, got_new={got_new}",
#                     "green",
#                 )
#                 return got_new
#             if timeout_sec is not None and (time.time() - t0) >= float(timeout_sec):
...
#                 transitions_to_save = sync_transition_list_grasp_penalty(
#                     sanitize_transition_list_for_storage(transitions, source=f"actor_buffer_save_step_{step}", print_summary=True),
#                     source=f"actor_buffer_save_step_{step}",
#                     penalty_value=FLAGS.grasp_penalty_value,
#                     print_summary=True,
#                 )
#                 demo_to_save = sync_transition_list_grasp_penalty(
#                     sanitize_transition_list_for_storage(demo_transitions, source=f"actor_demo_buffer_save_step_{step}", print_summary=True),
#                     source=f"actor_demo_buffer_save_step_{step}",
#                     penalty_value=FLAGS.grasp_penalty_value,
#                     print_summary=True,
#                 )
...
#                     pkl.dump(demo_to_save, f)
#                 demo_transitions = []
#                 _log_info("checkpoint", f"[actor-buffer-save] step={step}, buffer_saved={len(transitions_to_save)}, demo_saved={len(demo_to_save)}", "green")
...
#             buffer_path = os.path.join(FLAGS.checkpoint_path, "buffer")
#             os.makedirs(buffer_path, exist_ok=True)
#             with open(os.path.join(buffer_path, f"transitions_{final_step}_final.pkl"), "wb") as f:
#                 pkl.dump(transitions, f)
#         if demo_transitions:
#             demo_buffer_path = os.path.join(FLAGS.checkpoint_path, "demo_buffer")
#             os.makedirs(demo_buffer_path, exist_ok=True)
#             with open(os.path.join(demo_buffer_path, f"transitions_{final_step}_final.pkl"), "wb") as f:
#                 pkl.dump(demo_transitions, f)
#         print_yellow("[actor-exit] actor loop exited, remaining local buffers flushed.")
...
#     if FLAGS.demo_path is not None:
#         demo_files = resolve_demo_paths(FLAGS.demo_path)
#         _, _, sample_obs, sample_action = build_spaces_and_samples_from_demos(demo_files, config, actor_to_learner_image_keys)

#     agent, include_grasp_penalty = _make_agent_and_buffers(config, env, rng, sample_obs=sample_obs, sample_action=sample_action)
...
#         assert FLAGS.demo_path is not None, "learner 必须提供 --demo_path"
#         demo_files = resolve_demo_paths(FLAGS.demo_path)
#         for path in demo_files:
#             with open(path, "rb") as f:
#                 transitions = pkl.load(f)
...
#                 print_summary=True,
#             )
#             transitions = sanitize_transition_list_for_storage(transitions, source=f"demo_load:{path}", print_summary=True)
#             transitions = sync_transition_list_grasp_penalty(
#                 transitions,
#                 source=f"demo_load:{path}",
#                 penalty_value=FLAGS.grasp_penalty_value,
...
#                     transitions = pkl.load(f)
#                 transitions = prune_transition_list_for_actor_to_learner(
#                     transitions, actor_to_learner_image_keys, config, source=f"buffer_load:{file}", strict=FLAGS.actor_to_learner_strict_keys, print_summary=False
#                 )
#                 transitions = sanitize_transition_list_for_storage(transitions, source=f"buffer_load:{file}", print_summary=False)
#                 transitions = sync_transition_list_grasp_penalty(transitions, source=f"buffer_load:{file}", penalty_value=FLAGS.grasp_penalty_value, print_summary=False)
#                 for transition in transitions:
#                     replay_buffer.insert(transition)
...
#                     transitions = pkl.load(f)
#                 transitions = prune_transition_list_for_actor_to_learner(
#                     transitions, actor_to_learner_image_keys, config, source=f"demo_buffer_load:{file}", strict=FLAGS.actor_to_learner_strict_keys, print_summary=False
#                 )
#                 transitions = sanitize_transition_list_for_storage(transitions, source=f"demo_buffer_load:{file}", print_summary=False)
#                 transitions = sync_transition_list_grasp_penalty(transitions, source=f"demo_buffer_load:{file}", penalty_value=FLAGS.grasp_penalty_value, print_summary=False)
#                 for transition in transitions:
#                     demo_buffer.insert(transition)
#             print_green(f"Loaded previous demo buffer data. Demo buffer size: {len(demo_buffer)}")

#         print_green("starting learner loop")
#         learner(sampling_rng, agent, replay_buffer, demo_buffer=demo_buffer, wandb_logger=wandb_logger)
...
# # 动作卡顿还可以接受，但是卡顿期间的vr遥操数据不会被记录，非常影响训练 
# flags.DEFINE_integer("client_update_period", 1100, "How many actor env steps between explicit client.update() calls.")
