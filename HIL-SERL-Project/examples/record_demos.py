...
#     注意：
#     - 这里不要依赖 env.action_space.low/high。
#       你之前 action_space 打印过 Box(-inf, inf)，所以必须显式写死 [-1, 1]。
#     - 这个函数是幂等的，重复调用不会把 action 越变越小。
#     """
#     a = np.asarray(action, dtype=np.float32).reshape(-1).copy()
...
#         else:
#             raw_actions = np.zeros(env.action_space.shape, dtype=np.float32)
#             raw_source = "zero_no_intervention"

#         # 第一层保险：先把 raw action 的前 6 维 clip 到 [-1,1]
#         raw_actions = sanitize_single_arm_action_for_storage(
#             raw_actions,
#             quantize_gripper=False,
#             source=raw_source,
...
#         # 第二层保险：最终写入 transition 前，再确保完整 action 合法
#         actions = sanitize_single_arm_action_for_storage(
#             actions,
#             quantize_gripper=True,
#             source="after_gripper_rewrite",
...
#         # 第三层保险：transition 入口再 sanitize 一次
#         transition = sanitize_transition_for_storage(transition)
...
#                 clean_trajectory = []
#                 for trans in trajectory:
#                     clean_trajectory.append(sanitize_transition_for_storage(trans))
...
#     # 保存前最终全局保险
#     transitions = [sanitize_transition_for_storage(t) for t in transitions]
...
#     file_name = os.path.join(
#         save_dir,
#         f"{FLAGS.exp_name}_{success_needed}_demos_official_style_clean_{uuid}.pkl",
#     )
